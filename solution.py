...
# Put ALL checkpoint files next to solution.py in the zip root
GRU_CKPTS = [
...
# Balanced architecture weights:
# 4 GRUs get 50% total (0.125 each)
# 1 LSTM gets 25%
# 1 BiLSTM gets 25%
GRU_WEIGHT = 0.125      # per GRU
LSTM_WEIGHT = 0.25
BILSTM_WEIGHT = 0.25
...
BILSTM_WINDOW = 100  # BiLSTM window size
...
class PredictionModel:
    """
    Streaming ensemble inference with 6 models:
    - 4 GRUs (weight 0.125 each = 0.5 total)
    - 1 LSTM (weight 0.25)
    - 1 BiLSTM (weight 0.25)
    """
    ...
    def __init__(self):
        ...
        # BiLSTM needs to store history for windowing
        self.bilstm_history = []
    ...
    def predict(self, data_point):
        ...
        # === GRU predictions (use raw state only, no deltas in streaming) ===
        ...
        # For streaming, we need previous state to compute delta
        if len(self.bilstm_history) > 0:
            x_prev = self.bilstm_history[-1]
            dx = x_raw - x_prev
        else:
        ...
        # === BiLSTM predictions (needs window) ===
        # Add current state to history
        self.bilstm_history.append(x_raw.copy())
        
        if len(self.bilstm_history) >= BILSTM_WINDOW:
            # Get last WINDOW states
            window_states = self.bilstm_history[-BILSTM_WINDOW:]
            ...
            # Not enough history - use zeros or last available prediction
            bilstm_pred = np.zeros(2, dtype=np.float32)
        
        # === Weighted ensemble ===
        # 4 GRUs: 0.125 each (0.5 total)
        # 1 LSTM: 0.25
        # 1 BiLSTM: 0.25
